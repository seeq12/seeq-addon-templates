...
from myOldAddOn.utils import copy_notebooks

NB_EXTENSIONS = ['widgetsnbextension', 'plotlywidget', 'ipyvuetify', 'ipyvue']
DEPLOYMENT_FOLDER = 'deployment'
DEPLOYMENT_NOTEBOOK = "myOldAddOn_deployment.ipynb"
DEFAULT_GROUP = ['Everyone']
DEFAULT_USERS = []
ADDON_NAME = "My Old Add-on"
...
def cli_interface():
    """ Installs OldAddOn as a Seeq Add-on Tool """
    parser = argparse.ArgumentParser(description='Install OldAddOn as a Seeq Add-on Tool')
    ...
    parser.add_argument('--users', type=str, nargs='*', default=[],
                        help="List of the Seeq users to will have access to the OldAddOn"
                             " Add-on Tool, default: %(default)s")
    parser.add_argument('--groups', type=str, nargs='*', default=['Everyone'],
                        help="List of the Seeq groups to will have access to the OldAddOn"
                             " Add-on Tool, default: %(default)s")
...
if __name__ == '__main__':
    ...
    if project_id is None:
        ...
        if not project_id:
            raise RuntimeError(f'Could not install "seeq-myoldaddon" because the SDL project ID could not be found')
    ...
    print(f"\nThe OldAddOn Tool will be installed on the SDL "
